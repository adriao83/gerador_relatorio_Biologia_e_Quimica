    ...
    if op in mapa or op in ["2", "3"]:
        ...
        if op == "2":
            ...
            for i in ["2.1", "2.2", "2.3"]:
                ...
                while True:
                    ped = input(f"O que a IA escreve para {sub_t}?\n> ")
                    if input("Confirma pedido? (s/n): ").lower() == 's':
                        print(f"Processando {i}...")
                        res = chamar_minha_ia(f"No tema {dados['titulo_secao_2']}, escreva {i} {sub_t}: {ped}", modelo_ativo)
                        texto_l, refs_l = processar_e_extrair_refs(res, dados)
                        linhas_texto = texto_l.split('\n')
                        if linhas_texto[0].strip().startswith(i): texto_l = '\n'.join(linhas_texto[1:]).strip()
                        if input("\nSalvar esta subseção? (s/n): ").lower() == 's':
                            acumulado += f"\n{i} {sub_t.upper()}\n{texto_l}\n"
            ...
            # AQUI ESTÁ A MUDANÇA: TÍTULOS EM NEGRITO E QUEBRA DE LINHA
